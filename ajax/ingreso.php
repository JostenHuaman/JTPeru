...
/*
		...
		while ($reg = $rspta->fetch_object()) {
			...
			$botones = '<button class="btn btn-warning btn-xs" onclick="mostrar(' . $reg->idingreso . ')"title="Visualizar"><i class="fa fa-eye"></i></button>';
			if ($reg->estado == 'Aceptado') {
				$botones .= ' <button class="btn btn-danger btn-xs" onclick="anular(' . $reg->idingreso . ')"title="Cancelar"><i class="fa fa-close"></i></button>';
